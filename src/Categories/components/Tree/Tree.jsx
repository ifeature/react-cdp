...
function collapseCategories(categories, allCategories) {
    categories.forEach(category => {
        const cat = allCategories.find(c => c._id === category);
        cat.expanded = false;
    ...
    renderTree(data, parent = null) {
        ...
        return (
            <ul className={listClassName}>
                {
                    data.map(category => {
                        const categories = category.categories.map(id => this.props.data.find(c => c._id === id));
                        ...
                        return (
                            ...
                            <li key={category.id} className="categories__item">
                                <Category
                                    ...
                                    onCategoryAddRequest={this.props.onCategoryAddRequest.bind(this, category._id)}
                                    onCategoryEditRequest={this.props.onCategoryEditRequest.bind(this, category)}
                                    onCategoryExpand={handleCategoryExpand}
                                />
                                { expandable && expanded && this.renderTree(categories, category._id) }
                            </li>
                        );
                    })
                }
